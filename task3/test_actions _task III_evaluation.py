...
def calculate_reward(rob, last_stuck, current_stuck, last_red_percentage, current_red_percentage, green_percentage, last_distance_food_base, current_distance_food_base, last_distance_food_robot, current_distance_food_robot, last_position, current_position, base_detects_food, left_speed, right_speed):
    speed_reward = 0
    green_reward = 0
    red_reward = current_red_percentage * 5000 #always red reward
    ...
    if last_red_percentage >= 0.01 and current_red_percentage <= 0.0001 and current_distance_food_robot < 0.20: #first catch
        food_catch_reward += 200
    elif last_distance_food_robot < 0.20 and current_distance_food_robot < 0.20: #continue catch
        food_catch_reward += 200
    
    if left_speed > 0 and right_speed > 0: #forward speed
        speed_reward += abs(left_speed)/10.0 + abs(right_speed)/10.0 
    elif left_speed < 0 and right_speed < 0: #backward speed
        speed_reward -= abs(left_speed)/10.0 + abs(right_speed)/10.0 
    
    #base distance reward and green reward only when green pixels in camera and forward speed
    if abs(last_distance_food_base - current_distance_food_base) > 0.01 and green_percentage > 0 and left_speed > 0 and right_speed > 0: 
        base_distance_reward += (last_distance_food_base - current_distance_food_base)*100
        green_reward += green_percentage*500
        speed_reward += abs(left_speed)/2.0 + abs(right_speed)/2.0 #extra forward speed reward
    ...
    if last_stuck and current_stuck:
        speed_reward *= -1 #reverse speed reward to help robot escape from wall
        
    if last_stuck and not current_stuck:
        print("escape")
        avoid_reward += 100
    
    print(f"green_reward:{green_reward} , red_reward:{red_reward}, food_catch_reward:{food_catch_reward}")
    reward = base_distance_reward  + base_detects_food*150.0 + food_catch_reward + spin_penalty + stuck_penalty + green_reward + red_reward + speed_reward + avoid_reward
    
    scaled_reward = reward / 500.0 ##Scale the theoretical range (-220,500) to the range (-0.24, 1).
...
def eval_step(rob, action, move_index):
    ...
    move_index += 1
    done = move_index == 200 or rob.base_detects_food()
    
    return state, done, move_index

def eval_policy(rob, policy):
    ...
    for i in range(3):
        ...
        while not done:
            ...
            if done:
                ...
                fitness = 200 * rob.base_detects_food() - move_index 
                avg_fitness += fitness
        rob.stop_simulation()      
    avg_fitness /= 3
...
def run_all_actions(rob: IRobobo):
    log_file = RESULT_DIR / f"evaluation results.txt"
    for i in range(1, 186+1):  
            ...
            avg_fitness = eval_policy(rob, policy)
