...
def calculate_reward(rob, last_position, current_position, current_left_speed, current_right_speed, last_stuck, current_stuck):
    ...
    if (current_left_speed >=0 and current_right_speed >=0) and (abs(abs(current_left_speed)-abs(current_left_speed))<=10):
        linear_speed_reward = abs(current_left_speed)/2.0 + abs(current_right_speed)/2.0 #forward linear speed
    elif (current_left_speed <=0 and current_right_speed <=0) and (abs(abs(current_left_speed)-abs(current_left_speed))<=10):
        linear_speed_reward = abs(current_left_speed)/4.0 + abs(current_right_speed)/4.0 #backward linear speed  
    else:
    ...
    for i in range(3):
        ...
        while not done:
            ...
            if is_stuck(last_position, current_position):
                ...
                n_spin += 1
            elif is_backward(action):
                n_backward += 1

        rob.stop_simulation()      
        fitness = 100 - n_stuck * 10 - n_spin * 2 - n_backward
    avg_fitness+=fitness
...
def run_all_actions(rob: IRobobo):
    log_file = RESULT_DIR / f"evalution results.txt"
    for i in range(1, 25):  
